      ...
      <aside className="flex w-64 shrink-0 flex-col border-r border-border-soft bg-surface">
        <div className="space-y-2 p-3">
          <button
            ...
            className="flex w-full items-center gap-2 rounded-lg border border-border px-3 py-2 text-[13px] font-medium transition-colors hover:bg-surface-2"
          >
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="搜索会话"
              className="w-full rounded-lg border border-transparent bg-surface-2 py-1.5 pl-7 pr-2 text-[12.5px] outline-none transition-colors placeholder:text-text-faint focus:border-accent/50"
            />
          </div>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-2">
          {groups.map((g) => (
            <div key={g.label} className="mb-3">
              <div className="px-2 py-1 text-[11px] font-medium uppercase tracking-wide text-text-faint">{g.label}</div>
              {g.threads.map((t) => (
                <div
                  key={t.id}
                  className={`group flex items-center rounded-lg pr-1 transition-colors ${
                    t.id === state.threadId ? 'bg-surface-2' : 'hover:bg-surface-2/60'
                  }`}
                >
                  <button
                    type="button"
                  ...
                  >
                    {t.pinned && <span className="mr-1 text-text-faint">📌</span>}
                    {t.title || '新会话'}
                  </button>
                  <div className="flex shrink-0 items-center opacity-0 transition-opacity group-hover:opacity-100">
                    <button type="button" title="置顶" onClick={() => void togglePin(t)} className="rounded px-1 text-[11px] text-text-faint hover:text-text">📌</button>
                    <button type="button" title="重命名" onClick={() => void rename(t.id, t.title)} className="rounded px-1 text-[11px] text-text-faint hover:text-text">✎</button>
                    <button type="button" title="删除" onClick={() => void del(t.id)} className="rounded px-1 text-[11px] text-text-faint hover:text-danger">🗑</button>
                  </div>
                </div>
              ))}
            </div>
          ))}
          {groups.length === 0 && <div className="px-3 py-6 text-center text-[12px] text-text-faint">暂无会话</div>}
        </div>
        <div className="border-t border-border-soft p-2">
          <button
            type="button"
            onClick={() => setSettingsOpen(true)}
            className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-[13px] text-text-dim transition-colors hover:bg-surface-2 hover:text-text"
          >
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center gap-2 border-b border-border-soft px-5 py-3">
          ...
          <button
            ...
            className="ml-auto rounded-lg px-2.5 py-1 text-[12px] text-text-dim transition-colors hover:bg-surface-2"
        ...
        <aside className="w-60 shrink-0 border-l border-border-soft bg-surface p-4">
          ...
          {state.todos.length > 0 && (
            <div className="mb-4 space-y-1.5 text-[12.5px]">
              {state.todos.map((t, i) => (
                <div key={i} className={`flex gap-2 ${t.done ? 'text-text-faint line-through' : ''}`}>
                  <span className={t.done ? 'text-ok' : 'text-text-faint'}>{t.done ? '☑' : '☐'}</span>
                  <span>{t.text}</span>
                </div>
              ))}
            </div>
          ...
          {state.lastUsage ? (
            <div className="space-y-3 text-[12px]">
              <div>
                ...
                <div className="h-1.5 overflow-hidden rounded-full bg-surface-2">
                  <div className="h-full rounded-full bg-accent transition-[width]" style={{ width: `${state.lastUsage.contextPct}%` }} />
                </div>
              </div>
            ...
            <div className="text-[12px] text-text-faint">本轮暂无用量数据</div>
          )}
          {state.queuedInputs > 0 && <div className="mt-3 text-[12px] text-text-dim">队列 {state.queuedInputs} 条</div>}
        </aside>
      )}
