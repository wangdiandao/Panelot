  ...
  return (
    <div className="max-w-2xl space-y-3">
      ...
      {showImport && (
        <div className="space-y-2 rounded-[10px] border border-border p-3">
          <textarea
            ...
            className="w-full rounded-md border border-border bg-surface-2 p-2 font-mono text-[12px] outline-none focus:border-accent/60"
          />
          <div className="text-[11px] text-text-dim">兼容 Claude Code mcpServers / Cursor 配置片段（识别 url / type / headers.Authorization）。</div>
          {error && <div className="text-[12px] text-danger">{error}</div>}
          <button type="button" onClick={() => void doImport()} className="rounded-md border border-border px-3 py-1 text-[12px] hover:bg-surface-2">解析并添加</button>
        </div>
      )}
      ...
      {servers.length === 0 ? (
        ...
        servers.map((s) => (
          <div key={s.id} className="flex items-center gap-3 rounded-[10px] border border-border bg-surface px-4 py-3">
            <span className={`h-2 w-2 rounded-full ${s.enabled ? 'bg-ok' : 'bg-text-dim'}`} />
