...
> 上级文档：[DESIGN.md](../DESIGN.md)。本文沉淀 2026-07 对七个开源项目的调研结论：每项「采纳了什么、为什么、落在哪份文档」，以及踩坑警示。
...
| 采纳 | 理由 | 落点 |
|---|---|---|
| loop = 循环到无工具调用；不加 maxSteps 硬旋钮 | 「留下什么比加入什么重要」；原设计的硬步数上限改为软提醒 | 04 §1-2、[10 §7](./10-prompts.md) |
...
| CDP 独有能力清单（getEventListeners、pierce、setFileInputFiles） | L2 的杀手级用途依据 | 05 §2 |
