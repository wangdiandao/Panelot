...
> 文档入口：[用户指南](../guide/index.md) · 关联：[Agent 引擎](./agent-engine.md) · [权限](./permissions.md) · [提示词](./prompts.md)
...
浏览器工具、内置工具、交互工具和 MCP 工具进入引擎前都经过同一个 `ToolRegistry` 注册边界。工具工厂只声明一次 schema、level、effect、recovery、结果信任来源和 execution binding；Provider schema、审批展示、恢复快照不能维护平行副本。自动 L1→L2 升级仍为两个独立的稳定 tool call：升级工具使用自己的规范化 descriptor，再次经过 Gatekeeper 与派发前目标复验，并在执行前原子持久化匹配的 pendingTool。
...
工具的 `level:'L2'` 会在审批卡片已经出现时附带 `escalation_l2` 提示，但当前 Gatekeeper 不把“使用 debugger”本身作为强制审批条件：默认策略下 L2 read（如 screenshot）直接放行，`auto` 下未命中其它规则的 L2 write 也可放行。准确裁决语义见[权限](./permissions.md) §5。
...
当前注册：`fetch_url`（后台抓取→正文文本化）、`memory_read`、`memory_write`、`load_skill`（详见 [Skills 与 Plugins](./skills-plugins.md)）、`download`、`artifact`，以及统一交互工具 `ask_user`、`request_user_action`、`watch_page`、`schedule_resume`。`artifact` 生成 UTF-8 文件，保存到当前 Thread 的 attachments 后触发下载。
...
失败自纠标准路径（写入工具描述，详见[提示词](./prompts.md) §3）：ref 过期或找不到 → 重新 `read_page` → 用新 ref 重试；仍失败 → 换用当前可用的 L2 工具，或通过 `ask_user` 向用户说明并提问。

内容脚本通道在写操作派发后、回复前断开时，结果按「可能已生效」处理，不会自动重放 `click`、`type`、`select_option` 等写操作。只读请求在确认未改变页面的前提下可重新注入执行器并重试一次。
