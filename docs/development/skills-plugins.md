...
> 文档入口：[用户指南](../guide/index.md) · 关联：[Agent 引擎](./agent-engine.md) · [界面](./ui.md) · [提示词](./prompts.md)
...
1. 所有 enabled Skill 的 `name + description` 会进入 system prompt 的 Skills 区块，详见[提示词](./prompts.md) §6。带 `sites` 的 Skill 只在当前默认网页 tab 匹配时进入索引；显式 `tabId` 工具调用仍按实际目标经过工具与权限层裁决；
...
命令来自 Skill 与 MCP Prompt，统一注册到命令面板，详见[界面](./ui.md) §5：
...
变量表单：`VariableDef { key, label, type: 'text'|'select'|'date'|'url', options?, default?, required? }`，提交时替换命令模板中的 <code v-pre>{{key}}</code>。
