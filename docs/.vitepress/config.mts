...
export default defineConfig({
  ...
  cleanUrls: true,
  lastUpdated: true,
  // The policy wrappers include one shared source; only the wrappers should become routes.
  srcExclude: ['privacy-policy.md', 'privacy/_policy.md'],
  sitemap: {
    hostname: 'https://wangdiandao.github.io/Panelot/',
  },
  head: [
  ...
  themeConfig: {
    nav: [
      ...
      { text: 'GitHub', link: 'https://github.com/wangdiandao/Panelot' },
    ],
    sidebar: {
      '/guide/': [
        {
          ...
          items: [
            ...
            {
              text: '浏览器操作与权限',
              link: '/guide/browser-and-permissions',
            },
            { text: 'Provider 与模型', link: '/guide/providers-and-models' },
            {
              text: 'Skills、Plugins 与 MCP',
              link: '/guide/skills-plugins-mcp',
            },
    ...
    socialLinks: [{ icon: 'github', link: 'https://github.com/wangdiandao/Panelot' }],
    editLink: {
      pattern: ({ filePath }) => {
        const sourcePath =
          filePath === 'index.md' || filePath === 'privacy/index.md'
            ? 'privacy/_policy.md'
            : filePath;
        return `https://github.com/wangdiandao/Panelot/edit/main/docs/${sourcePath}`;
      },
      text: '在 GitHub 上编辑此页',
    },
    lastUpdated: {
