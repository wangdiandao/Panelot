...
const STRINGS: Record<string, { 'zh-CN': string; en: string }> = {
  ...
  'app.taskPanelHint': { 'zh-CN': '输入 /plan 让 AI 制定执行计划', en: 'Type /plan for the AI to outline a plan' },
  // Plan confirm card
  'plan.ready': { 'zh-CN': '计划已就绪，请确认后开始执行', en: 'Plan ready — confirm to start execution' },
  ...
  'plan.confirmMsg': { 'zh-CN': '计划已确认，请按步骤执行。', en: 'Plan confirmed, please proceed step by step.' },
  'cmd.planHint': { 'zh-CN': '让 AI 先制定分步计划，再逐步执行', en: 'Ask the AI to outline a step-by-step plan before acting' },
  'cmd.planPrompt': { 'zh-CN': '请先用中文制定一份分步执行计划，每步用 todo_write 工具写入任务列表，计划确认后再开始执行。', en: 'Please outline a step-by-step plan first. Write each step into the task list using todo_write, then begin execution once the plan is confirmed.' },
  ...
  'error.context_too_long': { 'zh-CN': '上下文超长 — 试试新会话', en: 'Context too long — try a new chat' },
  ...
  'keys.recallLast': { 'zh-CN': '召回上一条消息（空输入框）', en: 'Recall last message (empty composer)' },
  ...
  'perm.planHint': { 'zh-CN': 'AI 先制定计划并等待确认，确认后再执行操作', en: 'AI plans first and waits for your approval before acting' },
  'perm.always': { 'zh-CN': '全程询问', en: 'Ask for everything' },
  'perm.alwaysHint': { 'zh-CN': '每一步都先征求同意，包括读取页面', en: 'Every step asks first, reading included' },
  'perm.balanced': { 'zh-CN': '操作询问', en: 'Ask before acting' },
  'perm.balancedHint': { 'zh-CN': '自由读取；点击/输入等写操作先询问（默认）', en: 'Reads freely; clicks/typing ask first (default)' },
  'perm.auto': { 'zh-CN': '无需审批', en: 'Act without asking' },
  'perm.autoHint': { 'zh-CN': '自动执行操作；敏感站点与敏感信息仍会拦截', en: 'Acts autonomously; sensitive sites & payloads still guarded' },
  ...
  'model.noneHint': { 'zh-CN': '先添加一个模型连接，即可开始对话', en: 'Connect a provider to start chatting' },
  ...
  'empty.sugSummarizeHint': { 'zh-CN': '@ 引用页面后提炼要点', en: 'Attach the page and distill key points' },
  'empty.sugCompare': { 'zh-CN': '跨标签页比价/比参数', en: 'Compare across open tabs' },
  'empty.sugCompareHint': { 'zh-CN': '让 Panelot 逐个读取打开的标签页', en: 'Panelot reads each open tab for you' },
  'empty.sugForm': { 'zh-CN': '帮我填这个表单', en: 'Fill this form for me' },
  'empty.sugFormHint': { 'zh-CN': '每步写操作都会先征求批准', en: 'Every write action asks for approval first' },
  'empty.sugExtract': { 'zh-CN': '提取页面数据为表格', en: 'Extract page data as a table' },
  'empty.sugExtractHint': { 'zh-CN': '结构化抽取正文/列表/表格', en: 'Structured extraction of content/lists/tables' },
  ...
  'actions.editHint': { 'zh-CN': 'Esc 取消 · Ctrl+Enter 重新发送', en: 'Esc to cancel · Ctrl+Enter to resend' },
