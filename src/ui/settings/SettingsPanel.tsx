          ...
          {SECTIONS.map((s) => (
            <button
              ...
              className={`flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-[13px] transition-colors ${
                section === s.id ? 'bg-muted font-medium text-foreground' : 'text-muted-foreground hover:bg-muted/60 hover:text-foreground'
              }`}
            >
...
function GeneralPage() {
  ...
  const input = 'w-full rounded-lg border border-border bg-muted px-3 py-2 text-[13px] outline-none transition-colors focus:border-primary/60';

  return (
    <div className="max-w-xl space-y-5">
      <h2 className="text-[16px] font-semibold">通用</h2>
      <div>
        <label className="mb-1.5 block text-[12px] font-medium text-muted-foreground">全局自定义指令</label>
        <textarea
          className={input}
          rows={4}
        ...
        />
        <p className="mt-1 text-[11px] text-faint-foreground">拼入 system prompt 的用户层，对所有会话生效。</p>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="mb-1.5 block text-[12px] font-medium text-muted-foreground">语言</label>
        ...
        <div>
          <label className="mb-1.5 block text-[12px] font-medium text-muted-foreground">主题</label>
