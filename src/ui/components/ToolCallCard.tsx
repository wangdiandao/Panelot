...
export function ToolCallCard({ card }: { card: ToolCardData }) {
  ...
  return (
    <div className="rounded-[10px] border border-border bg-surface text-[12.5px]">
      <button
        type="button"
      ...
      >
        ...
        {card.paramsSummary && (
          <span className="truncate font-mono text-[11px] text-text-dim">{card.paramsSummary}</span>
        )}
      ...
      {expanded && (
        <div className="space-y-2 border-t border-border px-3 py-2">
          {card.params !== undefined && (
            <pre className="max-h-40 overflow-auto rounded bg-surface-2 p-2 font-mono text-[11px] text-text-dim">
              {JSON.stringify(card.params, null, 2)}
            </pre>
          )}
          {card.resultText && (
            <pre className="max-h-60 overflow-auto whitespace-pre-wrap rounded bg-surface-2 p-2 font-mono text-[11px]">
              {card.resultText}
            </pre>
          )}
        </div>
  ...
  return (
    <div className="space-y-1">
      <button
        type="button"
        onClick={() => setExpanded((e) => !e)}
        className="flex w-full items-center gap-2 rounded-[10px] border border-border bg-surface px-3 py-1.5 text-left text-[12.5px] text-text-dim hover:bg-surface-2"
        aria-expanded={expanded}
      >
        <span>{expanded ? '▾' : '▸'}</span>
        <span>
          {cards.length} 步操作 <span className="text-ok">✓{okCount}</span>
